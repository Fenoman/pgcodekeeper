...
- Повторное `Получить изменения` для PostgreSQL переиспользует структурную модель неизменившегося проекта и уменьшает нагрузку на CPU. Перед повторным использованием проверяется содержимое всех SQL-файлов, включая внешние правки с сохранёнными размером и временем файла. Изменение проекта, настроек или глубины сравнения требует новой загрузки; база читается при каждом запуске.
- При первом структурном сравнении проверка прочитанных файлов проекта выполняется параллельно с загрузкой базы. Хеширование использует один буфер на проход и создаёт меньше временных объектов; финальная проверка списка файлов и изменений проекта остаётся после завершения обеих сторон.
- При `Получить изменения` для PostgreSQL модели сравнения больше не собирают карты расположений ссылок, что уменьшает расход памяти. Анализ зависимостей, миграционные скрипты и индекс навигации SQL-редактора сохраняются.
- Core обновлён до `15.3.0-neo2`: рабочие соединения каталога подготавливаются параллельно, кэш индексов и триггеров переиспользует неизменившийся снимок, данные настроек колонок передаются компактнее, а декодирование строк кэша создаёт меньше временных объектов.

### Исправлено

- SWTBot-тест редактора приведён в соответствие с диалогом полного анализа: проверяется доступность полного анализа и отмена без создания папки миграции.

## [15.3.0-neo1] - 2026-08-31

### Добавлено

- Проект индексируется один раз, и индекс хранится на диске, поэтому открытие рабочего пространства больше не означает повторное чтение и разбор всех файлов. Всплывающие подсказки, структура файла, автодополнение и поиск объектов отвечают из индекса сразу после перезапуска.
- Сохранение файла или получение дневной порции изменений обновляет индекс проекта только по изменившимся файлам. Индекс, от которого рабочее дерево ушло вперёд, чинится по списку разошедшихся файлов, а не пересобирается целиком; сборка сообщает, какие это были файлы и почему выбран тот или иной путь. Файл, который не удалось разобрать, обходится в починку этого одного файла и отрисовку его маркеров ошибок, а не в пересборку; сборка, которую отменили или обогнало более новое состояние рабочего пространства, оставляет индекс следующей сборке; применение файла настроек проекта, не меняющее ни одного значения, тоже его сохраняет — одна сборка работает по одному чтению конфигурации, от перечисления файлов до публикации результата.
- Схему можно исключить из индекса проекта (`Окно -> Настройки -> pgCodeKeeper`, и отдельно для проекта в `Проект -> Свойства -> pgCodeKeeper`): по одному точному имени в строке, в проекте любого типа БД. Сравнение и генерация миграции не затрагиваются; исключённая схема теряет всплывающие подсказки и структуру файла. Имя, которое не смогла бы прочитать ни одна сборка, отклоняется при вводе, а изменение на уровне рабочего пространства доходит до уже открытых редакторов.
- Пачку, которая добавляет файлы, можно переиндексировать инкрементально вместо полной пересборки индекса проекта (`Окно -> Настройки -> pgCodeKeeper`, и отдельно для проекта). Настройка по умолчанию выключена и применяется только тогда, когда добавление доказуемо не может задеть уже накопленное индексом; всё остальное, как и любое удаление файла, по-прежнему уходит в полную сборку. Индекс строится под эту настройку, поэтому её переключение стоит одной полной сборки.
- Число потоков парсера задаётся отдельно для получения изменений и для индексирования проекта (`Окно -> Настройки -> pgCodeKeeper`). Введённое здесь значение принадлежит рабочему пространству и всегда имеет приоритет; если его не задавать, каждое из двух значений зависит от машины.
- Повторное сравнение с той же базой переиспользует проанализированный проект и снимок каталога от предыдущего запуска. Переиспользование отвергается и всё загружается заново, как только неизменность проекта, настроек или подключения нельзя доказать, — так устаревшая модель не может попасть в скрипт миграции.
- Сравнение проекта PostgreSQL больше не анализирует тело функции или процедуры, совпадающее с обеих сторон, — за счёт этого повторное сравнение и становится быстрее. Настройка по умолчанию включена и отключается глобально, для отдельного проекта или для одного запуска `Получить изменения с предустановками...`. Скрипт миграции из такого сравнения не строится никогда: при попытке его сформировать предлагается один раз пересчитать изменения с полным анализом, не меняя сохранённых настроек, а при применении изменений из такого сравнения диалог сообщает, что раскрытие зависимостей может быть неполным.
- Добавлен режим для проекта, который только принимает изменения из базы (`Проект -> Свойства -> pgCodeKeeper`, та же настройка есть и на уровне рабочего пространства). Такой проект сравнивается без анализа зависимостей и не индексируется в фоне — за счёт этого он и работает быстрее. Скрипт миграции по-прежнему строится из полностью проанализированного сравнения, поэтому он совпадает с тем, что даёт выключенный режим.
- Панель сравнения помечает каждую строку, которую не перенесёт ни одна миграция: фон под колонками, названными правилом игнорирования, буква на полях рядом со строкой, которую сравнение пропускает, и легенда над панелью, объясняющая каждую пометку и называющая колонки, к которым она относится. Легенда называет и сортировку колонки, которую не выразить в скрипте миграции, — такое различие не исчезнет никогда.
- Рядом со счётчиком сравниваемых объектов редактор теперь сообщает, сколько скрыли правила игнорирования, какие правила сработали и на чём. Правило, которое незаметно перестало срабатывать, больше не выглядит точно так же, как работающее.
- Дерево проекта держит секции партиционированной таблицы ниже таблиц своей папки и умеет показывать их под самой таблицей. Упорядочивание включено по умолчанию и ничего не перемещает: секция остаётся в своей папке и по своему пути, меняется только строка, в которой она стоит. Вложение по умолчанию выключено — оно действительно убирает секцию из папки, где она лежит. Папка с двумя тысячами секций превращается в восемьдесят файлов, которые действительно редактируют, а каждое семейство секций находится в одном клике.
- Автодополнение: точка после таблицы или псевдонима в SQL-редакторе предлагает колонки этого отношения — в том числе для псевдонима, который предложение `FROM` только объявило, и для псевдонима в отдельном SQL-файле, не принадлежащем ни одному проекту. Отношение ищется по ключу, а не перебором всех определений проекта, поэтому размер проекта не определяет цену нажатия клавиши. Текст короче триграммы больше не запрашивает дополнение по объектам ни в одной из реализаций, поэтому предлагаемое не зависит от того, построен ли индекс проекта; на ключевые слова и шаблоны это не влияет.
- Сборка проекта и запуск `Получить изменения` сообщают, на какой они фазе и как далеко она продвинулась, вместо неподвижной полосы.
- Добавлены три настройки сравнения: не учитывать кэш последовательности, не учитывать цель сбора статистики по колонке и убирать ключевое слово `ONLY`, которое отвергает гипертаблица TimescaleDB. Все три по умолчанию выключены и доступны в `Окно -> Настройки -> pgCodeKeeper`, в свойствах проекта, в `Мастере сравнения` и для одного запуска `Получить изменения с предустановками...`. Кэш последовательности предлагается для PostgreSQL и MS SQL, две другие — только для PostgreSQL.
- В `Окно -> Настройки -> pgCodeKeeper` добавлено управление кэшами: папки кэша каталога и кэша проанализированной модели, размер каждого (вычисляется в фоне) и одна кнопка, которая чистит оба и сообщает, сколько освободил каждый.
- Время сборки и сравнения пишется в собственный журнал производительности ограниченного размера в метаданных рабочего пространства, поэтому оно остаётся доступным, хотя самостоятельная сборка и держит журнал ошибок на уровне предупреждений.
- Самостоятельная сборка пишет дамп памяти при нехватке heap-памяти. Каталог создаётся в метаданных рабочего пространства и проверяется на запись при старте: рабочий каталог, который иначе взяла бы виртуальная машина, доступен только на чтение до конца запуска и находится внутри пакета приложения после него. Журнал производительности сообщает, можно ли вообще записать дамп.
- Самостоятельная сборка теперь собирается и публикуется также для Linux на Apple silicon.

### Изменено

- Фоновый индекс проекта больше не только для PostgreSQL: проект MS SQL или ClickHouse индексируется, восстанавливается после перезапуска и чинится инкрементально ровно так же, как проект PostgreSQL, поэтому всплывающие подсказки и структура файла больше не ждут полного разбора проекта. Индекс читает файл парсером того типа, которым является проект, и замер показывает, что он содержит то же, что полная загрузка. Настройки исключения схем и инкрементального переиндексирования при добавлении файлов по той же причине предлагаются для всех типов проектов. Перенос целиком проанализированной модели из одного сравнения в следующее по-прежнему доступен только для PostgreSQL: загрузчик, воспроизводящий сохранённый анализ, есть лишь у него.
- В Eclipse по умолчанию включено постоянное кэширование строк каталога PostgreSQL для сокращения трафика при повторных сравнениях. Кэш сохраняется после перезапуска приложения при использовании того же рабочего пространства; настройку можно отключить.
- Число потоков парсера по умолчанию теперь зависит от машины и одинаково в самостоятельных сборках и в Eclipse-плагине: половина доступных процессоров, но не менее двух и не более восьми. Настройка применялась только в самостоятельной сборке, поэтому плагин, установленный в другой Eclipse, её не получал.
- Максимальный размер heap-памяти самостоятельных сборок увеличен до 4 ГиБ при сохранении начального размера 256 МиБ; максимальный объём не выделяется заранее.
- Каждый архив самостоятельной сборки теперь несёт нужную ему Java. Её несли только архивы macOS, поэтому сборки для Windows и Linux работали на той Java, что оказалась установлена на машине.
- Самостоятельная сборка замечает файлы, которые пишут Git и внешние редакторы, и обновляет рабочее пространство сама, поэтому индекс проекта и редакторы видят их без ручного обновления. Плагин, установленный в чужой Eclipse, оставляет настройку этого Eclipse как есть: она задаёт поведение всех проектов рабочего пространства, а не только проектов pgCodeKeeper.
- Сайт обновлений разработчика больше не входит в поставку: его не предлагает ни самостоятельная сборка, ни установленный в Eclipse плагин. Оставшийся репозиторий Eclipse указывает на конкретный релизный поток, а не на то, во что `latest` разрешится в день обновления.
- Обе стороны панели сравнения строятся из одного ответа, а дочерние объекты упорядочены по типу, затем по имени. База возвращает индексы и ограничения в порядке каталога, а проект — в порядке файлов, и это показывалось как различие там, где оба состояния совпадали.
- Снятие фильтра по имени на большом сравнении больше не тормозит: таблица объектов платит за подсветку совпадений только пока фильтр задан. На одиннадцати тысячах объектов снятие фильтра, поймавшего один из них, занимало около минуты с зависшим окном; теперь это доли секунды, а заполнение таблицы после `Получить изменения` стало быстрее в двадцать раз.
- Задача полной пересборки индекса проекта видна в списке выполняемых операций и называет проект, который пересобирает, вместо того чтобы скрытно работать почти минуту.
- Подсказка настройки исключения схем говорит, чего именно стоит исключение: индекс — это то, что отвечает на всплывающие подсказки и структуру файла, поэтому исключённая схема перестаёт быть объяснимой в редакторе.

### Исправлено

- Сохранено наследование глобальных значений для отсутствующих проектных настроек, поэтому сохранение свойств проекта больше не может незаметно отменить глобальное отключение опции.
- Каждому, кто пишет файлы проекта, передаются те же правила, по которым скрывало сравнение. `Применить к проекту`, `Быстрое обновление` и редактор комментариев собирали эти правила отдельно или не собирали вовсе, поэтому нажатие `Применить к проекту` записывало в файл таблицы ровно те колонки, которые правило `type=COLUMN` только что скрыло из дерева.
- Создание проекта из базы идёт через выгрузку, применяющую правила игнорирования проекта, поэтому уже лежащий в каталоге файл правил и общий список рабочего пространства учитываются с первого записанного файла.
- Списки игнорирования собираются один раз, до начала сравнения. Список рабочего пространства, списки подключения к базе и дополнительные списки проекта доходили только до таблицы различий, поэтому дерево за ней строилось без них.
- Панель сравнения больше не отображает дочерние объекты, которые правило игнорирования вывело из сравнения, а скрытая колонка больше не считается изменением своей таблицы.
- Представление графа зависимостей объясняет, почему ему нечего показать, вместо пустого графа, и очищает строку состояния на любом пути выхода.
- Предупреждение в диалоге фиксации переносится по строкам, а не растягивает диалог под свою ширину.
- Самостоятельная сборка сохраняет то состояние рабочего пространства, которое поддерживает; удаляются только сохранённые элементы тех возможностей, которые она не поставляет.
- Правка проекта во время сравнения сообщается как таковая, и сравнение сразу помечается устаревшим, а не всплывает позже в виде посторонней ошибки.
